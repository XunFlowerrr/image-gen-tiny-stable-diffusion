...
from datasets import DownloadConfig, Features, Image as HFImage, Value, load_dataset
...
# Set streaming=False to make the dataset indexable
...
class HuggingFaceImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            input_ids = tok_out["input_ids"].squeeze() # Remove batch dim
            attention_mask = tok_out["attention_mask"].squeeze() # Remove batch dim
...
# Pass the dataset config ('2014') to handle caption extraction correctly
...
for epoch in range(999):                        # break via max_train_steps
    ...
    for imgs, ids, masks in progress:
        imgs  = imgs.to("cuda", dtype=vae_dtype, non_blocking=True)
...
pipe = StableDiffusionPipeline(
    ...
    feature_extractor = None       # must be passed when checker is None
...
# Define components for the matrix
...
for r_idx, subject in enumerate(subjects):
    for c_idx, setting in enumerate(settings):
        ...
        # Add labels to rows and columns for clarity
        if c_idx == 0:
